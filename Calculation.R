...
cal.mr = function(n,p.mr,per.m.in.ltfus,n.followyr,avg.ltfur){
  subjects = sample(n)
  ...
  for(yr in 1:n.followyr){
    ...
    if(n.ltfus<=length(subjects)){ # if no. of loss of follow-up is still smaller than th remain subjects (Did anyone still alive to be lost?)
    ...
    if(n.mistake <= n.ltfus){ # Did there anyone
      ...
      if(nominal.n.mos<=length(subjects)){ # if no. of extra mortality cases is still smaller than th remain subjects
        ...
        ptar = ptar+n.ltfus*(yr)+nominal.n.mos*(yr-0.5)   # add the contribution of person-year from each subject in ltfus & nominal.mos to ptar
      }
    }else{
      if(n.mos<=length(subjects)){ # if no. of mortality cases is still smaller than th remain subjects
        mos = sample(subjects,n.mos) # no. of mos 
        
        subjects = subjects[! subjects %in% mos] # remain only subjects who is not mos (2nd time to del subjects)
        
        n.total.nominal.mos = n.total.nominal.mos+n.mos
        ptar = ptar+n.ltfus*(yr)+n.mos*(yr-0.5)   # add the contribution of person-year from each subject in ltfus & mos to ptar
      }
    }
    
    n.total.mos = n.total.mos+n.mos # no matter how many the nominal mortality is, add the actual mos into n.total.mos
  }
